...
// C/C++ source code generated on : Thu Feb 08 20:32:24 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"my_sim\" **\n");
        ...
        rtmSetErrorStatus(my_sim_M, (NULL));
        my_sim_initialize();
...

        {
          ...
          rtExtModeWaitForStartPkt(my_sim_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(my_sim_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(my_sim_M)
                == (NULL)) || rtmGetStopRequested(my_sim_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(my_sim_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModePauseIfNeeded(my_sim_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(my_sim_M, true);
          }
    ...
    void NodeInterface::extmodeBackgroundTask(void)
    {
      while (mRunModel) {
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModeOneStep(my_sim_M->extModeInfo, 1,
                           &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(my_sim_M, true);
          }

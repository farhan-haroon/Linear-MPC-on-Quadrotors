...
// C/C++ source code generated on : Tue Feb 06 19:42:16 2024
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"test_sim\" **\n");
        ...
        rtmSetErrorStatus(test_sim_M, (NULL));
        test_sim_initialize();

        /* External mode */
        rtSetTFinalForExtMode(&rtmGetTFinal(test_sim_M));
        rtExtModeCheckInit(1);

        {
          boolean_T rtmStopReq = false;
          rtExtModeWaitForStartPkt(test_sim_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_sim_M, true);
          }
        }
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(test_sim_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(test_sim_M)
                == (NULL)) || rtmGetStopRequested(test_sim_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(test_sim_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModePauseIfNeeded(test_sim_M->extModeInfo, 1,
            &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_sim_M, true);
          }

          if (rtmGetStopRequested(test_sim_M) == true) {
    ...
    void NodeInterface::extmodeBackgroundTask(void)
    {
      while (mRunModel) {
        ...
        {
          boolean_T rtmStopReq = false;
          rtExtModeOneStep(test_sim_M->extModeInfo, 1,
                           &rtmStopReq);
          if (rtmStopReq) {
            rtmSetStopRequested(test_sim_M, true);
          }
